...
const showCohorts = (city, dataCohorts) => {
    mainSection.innerHTML = `<select style ="border-color: black; box-shadow: 0.2em 0.2em black;" class="form-control" id='select'></select>`
    ...
    cohortByCity.forEach(cohort => {
        ...
        options.innerHTML +=
            `<option style ="background: rgb(255, 229, 33);" id"${cohort.id}"=>${cohort.id}</option>`
    });
};
